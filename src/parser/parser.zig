...
pub fn parse(allocator: std.mem.Allocator, io: std.Io, base_path: []const u8, ignore_masks: ?[]const []const u8) !ParseResult {
    var proto_files = try paths.findProtoFiles(allocator, io, base_path, ignore_masks);
    defer {
    ...
    for (proto_files.items) |file_path| {
        var buffer = try ParserBuffer.initFile(allocator, io, file_path);
